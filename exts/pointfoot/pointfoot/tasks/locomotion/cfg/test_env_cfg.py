...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformVelocityCommandCfg(
        ...
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-1.0, 1.0), lin_vel_y=(-0.2, 0.2), ang_vel_z=(-0.5,0.5)
        ),
    )
...
class ObservarionsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        base_ang_vel = ObsTerm(func=mdp.base_ang_vel, noise=GaussianNoise(mean=0.0, std=0.2))
        proj_gravity = ObsTerm(func=mdp.projected_gravity, noise=GaussianNoise(mean=0.0, std=0.05))

        # robot joint measurements
        joint_pos = ObsTerm(func=mdp.joint_pos_rel, noise=GaussianNoise(mean=0.0, std=0.01))
        joint_vel = ObsTerm(func=mdp.joint_vel, noise=GaussianNoise(mean=0.0, std=1.5))
...
class EventsCfg:
    ...
    push_robot = EventTerm(
        ...
        mode="interval",
        interval_range_s=(7.0, 11.0),
        params={"velocity_range": {"x": (-0.5, 0.5), "y": (-0.2, 0.2), "yaw": (-math.pi / 6, math.pi / 6)}},
    )
...
class RewardsCfg:
    ...
    #     params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*foot_[LR]_Link"), "threshold": 350},
    #     weight=-0.01,
    # )
